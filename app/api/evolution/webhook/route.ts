...
async function salvarLog(
  ...
  await supabase
    ...
    .insert({
      cliente_id:
        params.vinculo?.cliente_id ||
        null,

      servico_id:
        params.vinculo?.servico_id ||
        null,
...
async function encaminharParaN8n(
  webhookUrl: string | null,
  body: any,
  vinculo: any
) {
  /*
    BLOQUEIA CLIENTE VENCIDO
  */

  if (
    servicoBloqueado(vinculo)
  ) {
    return {
      enviado: false,
      motivo:
        "cliente vencido/bloqueado",
    };
  }
...
export async function POST(
  ...
  try {
    ...
    let n8n: any = {
      ...
      motivo:
        "evento não encaminhado",
    };

    const ehMensagem =
      eventoNormalizado.includes(
        "MESSAGES_UPSERT"
      ) ||
      eventoNormalizado.includes(
        "MESSAGE"
    ...
    if (
      ...
      await notificarErroN8n({
        ...
        erro:
          n8n.erro ||
          n8n.motivo ||
          "Falha ao enviar mensagem para o n8n",
