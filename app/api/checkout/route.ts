...
async function validarCupomCheckout({
  ...
  if (
    cupom.servico_id &&
    cupom.servico_id !== servicoId
  ) {
    throw new Error(
      "Cupom não é válido para este serviço"
    );
  }

  if (
    cupom.plano_id &&
    cupom.plano_id !== planoId
  ) {
    throw new Error(
      "Cupom não é válido para este plano"
    );
  }
  ...
  try {
    ...
    if (!planoId || !servicoId) {
      return NextResponse.json(
        {
          error:
            "Selecione um serviço e um plano",
        },
        { status: 400 }
      );
    }
    ...
    try {
      ...
      descontoValor =
        cupomValidado.descontoValor;

      valorFinal =
        cupomValidado.valorFinal;
    } catch (error: any) {
      return NextResponse.json(
        {
          error: error.message,
